package Ascent.view.game;

import Ascent.gui.GUI;
import Ascent.model.entities.monster.Monster;
import Ascent.model.game.Position;
import org.junit.jupiter.api.BeforeEach;
import org.junit.jupiter.api.Test;
import org.mockito.Mockito;

/* this tester is basically the same as the playerviewer tester with some small adjustments */
class MonsterViewerTest {
    private MonsterViewer monsterViewer;
    private GUI gui;

    @BeforeEach
    void setup() {
        monsterViewer = new MonsterViewer();
        gui = Mockito.mock(GUI.class);
    }

    @Test
    void drawMonsterAtValidPosition() {
        Monster monster = new Monster(new Position(5, 8));
        monsterViewer.draw(monster, gui);
        Mockito.verify(gui).drawChar(5, 8, 'M', "RED");
    }

    @Test
    void drawMonsterAfterMovingOnce() {
        // simulate monster movement which is not implemented yet (to check if it is drawn properly)
        Monster monster = new Monster(new Position(5, 5));
        monster.moveDown();

        monsterViewer.draw(monster, gui);
        Mockito.verify(gui).drawChar(5, 6, 'M', "RED");
    }

    @Test
    void drawMonsterAfterMultipleMoves() {
        // same as the last function...
        Monster monster = new Monster(new Position(10, 10));

        monster.moveUp();
        monster.moveUp();
        monster.moveRight();
        monster.moveDown();
        monsterViewer.draw(monster, gui);

        // 10,10 -> 10,9 -> 10,8 -> 11,8 -> 11,9
        Mockito.verify(gui).drawChar(11, 9, 'M', "RED");
    }

    @Test
    void drawMonsterWithModifiedColor() {
        Monster monster = new Monster(new Position(5, 5));
        monster.setColor("GREEN");

        monsterViewer.draw(monster, gui);
        Mockito.verify(gui).drawChar(5, 5, 'M', "GREEN");
    }
}