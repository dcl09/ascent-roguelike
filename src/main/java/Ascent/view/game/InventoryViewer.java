...
public class InventoryViewer {
    ...
    public void draw(GUI gui, Player player, int x, int y) {
        ...
        // Weapon Section
        drawWeapon(gui, inventory.getEquippedWeapon(), x, y + 5);

        // Armour Section
        drawArmour(gui, inventory.getEquippedArmour(), x, y + 7);
    ...
    private void drawWeapon(GUI gui, Weapon weapon, int x, int y) {
        if (weapon != null) {
            String weaponText = String.format("Weapon: %s (+%d DMG)",
                    weapon.getName(), weapon.getBonusDamage());
            gui.drawText(x, y, weaponText, ITEM_COLOR);
        } else {
            gui.drawText(x, y, "Weapon: -", EMPTY_COLOR);
        }
    }
    ...
    private void drawArmour(GUI gui, Map<ArmourSlot, Armour> armourMap, int x, int y) {
        ...
        for (ArmourSlot slot : slots) {
            ...
            if (armour != null) {
                String armourText = slotName + armour.getName() + " (+" + armour.getBonusResistanceToDamage() + " ARM)";
                gui.drawText(x, y + offset, armourText, ITEM_COLOR);
            } else {
                gui.drawText(x, y + offset, slotName + "-", EMPTY_COLOR);
            }
            offset++;
        }
    }
    ...
    private void drawPotions(GUI gui, List<HealthRestore> potions, int x, int y) {
        gui.drawText(x, y, "--- Potions [0-9] ---", TITLE_COLOR);
        ...
        for (int i = 0; i < 5; i++) {
            ...
            if (i < potions.size()) {
                ...
                gui.drawText(x, y + 1 + i, potionText, ITEM_COLOR);
            } else {
                gui.drawText(x, y + 1 + i, slot + "-", EMPTY_COLOR);
            }
        }
    }
}
